...
```
### dfSummary
For smaller datasets that can be brought into memory in a reasonable amount of times, R packages such as `{summarytools}` can be useful for getting descriptive statistics with a minimum of coding.
```{r summarytools}
institutions_all <- dbGetQuery(
  con,
  "SELECT * FROM institutions"
)

summarytools::dfSummary(institutions_all)
```
...
### Data Quality
Data quality is not measured by a single value. Rather, it is measured within different dimensions, each dimension describing different characteristics of data.The metrics defined for each dimension are domain specific (i.e. what you will measure for a bibliographic work is different than what you will measure for an organizational record).
...
The following are is a list, albeit not an exhaustive list, of data quality metrics for the researcher graph.
